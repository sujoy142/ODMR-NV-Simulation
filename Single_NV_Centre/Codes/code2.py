...
B_fields = [0, 5, 10, 15]  # 0 to 15 G

# With strain (effective transverse E-field):
E0_strain = 20.6e6  # V/m
thetaE = np.pi / 2.0
phiE = 0.0
...
calculated_linewidth = 1.85 + (0.106 * np.sqrt(P_mw))
...
for idx, B0 in enumerate(B_fields):
    ...
    T_strength = simnv.ESR_singleNV(
        ...
        calculated_linewidth, hyperfine_on=False, mw_power_dbm=p_dbm
    )
    ...
    plt.plot(MWfreq, stacked_trace, linewidth=2.0, color=colors[idx])
